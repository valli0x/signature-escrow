...
- **SECURITY — verify before signing:** the acceptor REJECTS unless `LondonSigner(chainId=1).Hash(decode(tx_data))` equals the hash it is about to sign ("refusing to sign: tx hash does not match tx_data"). So a malicious initiator can't DISPLAY a benign tx while making us sign a different one (e.g. draining the escrow). The app also calls `/v1/tx/decode` to show the real to/amount. NOTE: each `accept` authorizes ONE tx from ONE account — draining two accounts needs two accepts.
