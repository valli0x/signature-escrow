...
- `GET/POST /v1/exchanges/{list,create,update,upsert,delete}` (client, `client/exchange.go`) — exchanges link TWO escrow accounts; PER-SIDE fields `address_a/partner_a/status_a` + `_b`. `upsert` create-or-replaces by id (acceptor imports a proposal under the initiator's id).
- `GET/POST /v1/aliases/{list,set,delete}` (client, `client/aliases.go`) — personal address-book labels (lowercased keys), shown for partners/addresses in the app.
- `POST /v1/tx/decode {network,tx_data}` (client, `client/tx.go`) — decodes an unsigned tx so the ACCEPTOR can verify the real to/value/nonce of what they co-sign (not the sender's claimed display fields).
- `POST /v1/escrow/check {id,pub}` (server, `server/escrow.go`) — read-only poll of a pollination; returns the released counterparty signature for `pub` once BOTH flowers validate, else `pending`. (`POST /v1/escrow` is the deposit.)
- `POST /v1/cosign/complete {hash,signature}` (client) — attaches the partner-returned signature to the initiator's 'sent' event so the initiator can broadcast too (app forwards it via a `sign-result` mailbox message).
...
- **SECURITY — verify before signing:** the acceptor REJECTS unless `LondonSigner(chainId=1).Hash(decode(tx_data))` equals the hash it is about to sign ("refusing to sign: tx hash does not match tx_data"). So a malicious initiator can't DISPLAY a benign tx while making us sign a different one (e.g. draining the escrow). The app also calls `/v1/tx/decode` to show the real to/amount. NOTE: each `accept` authorizes ONE tx from ONE account — draining two accounts needs two accepts; a per-swap single-co-sign guard + explicit confirm dialog were discussed as the next hardening step.

## Escrow atomic swap (no timebox yet)
- Pollination escrow (`server/escrow.go`) is a 2-party FAIR exchange: each side deposits a "flower" `{pub, hash, sig}` under a shared `id`; the server validates crosswise (`f2.sig` signs `f1.hash`/`f1.pub` and vice-versa) and releases each its OWN withdrawal sig only when BOTH are valid. Single flower → `pending`.
...
- OPEN: not yet live-verified that server `validation.Validate` accepts the 65-byte `SigEthereum` (r‖s‖v) format in pollination — test with a real two-party swap.
